...
class Server(FederatedTrainingDevice):
    ...
    def cluster_clients(self, S,number):
        pca = PCA(n_components=0.99).fit(S)
        S_new = pca.transform(S)
        clustering,u,u0,d,jm,p,fpc = cmeans(S_new,m=2,c=2,error=0.5,maxiter=10000)
        s1=np.zeros(S.shape[1])
        s2=np.zeros(S.shape[1])
        c1 = np.argwhere(clustering == 0).flatten()
        c2 = np.argwhere(clustering == 1).flatten()
        sum1=0
        sum2=0
        for i in range(len(c1)):
            sum1+=number[c1[i]]
            s1+=S[c1[i]]*number[c1[i]]
        s1/=sum1
        for i in range(len(c2)):
            sum2+=number[c2[i]]
            s2+=S[c2[i]]*number[c2[i]]
        s2/=sum2
        distance=np.dot(s1, s2) / (np.linalg.norm(s1) * np.linalg.norm(s2))
